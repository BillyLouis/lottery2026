...
// 	// --- SPINNER START ---
// 	stopSpinner := startSpinner("Processing...")
...
// 	log.Printf("Eigen predictor completed.")
// 	fmt.Println("---------------------------------------")

// }

// // startSpinner creates a goroutine that prints a spinning char.
// // It returns a function that, when called, stops the spinner.
...
// 				fmt.Printf("\r%c %s ", spinChars[i], msg)
...
func main() {
	...
	// --- Create Simulator ---
	stopSim := startSpinner("Init Simulator...")
	simulator := sim.NewSimulator()
	stopSim()

	// --- Create Monte Carlo Model with Seed ---
	stopMC := startSpinner("Init Monte Carlo...")
	//time.Sleep(1 * time.Second)
	mc := model.NewMonteCarloModel(feats, simulator, seedMode)
	stopMC()

	// --- Build Ensemble ---
	stopEns := startSpinner("Building Ensemble...")
	ens := model.NewEnsemble()
	ens.AddModel(model.NewFrequencyModel(feats)) // frequency model
	ens.AddModel(mc)                             // Monte Carlo model
	stopEns()
	...
	stopEigen := startSpinner("Processing Eigen...")
	simulator.RunPredictor()
	stopEigen()
